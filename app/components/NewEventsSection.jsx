...
import React, { useState } from "react";
import styles from "./NewEventsSection.module.css";
    ...
    events: [
      {
        ...
        Point: "Google Cloud Study Jams: Hands-on Workshop",
        photos: [
          "assets/images/cyber1.jpeg",
          "assets/images/cyber2.jpeg",
          "assets/images/cyber3.jpeg",
          "assets/images/cyber4.jpeg",
        ],
      },
      {
        Date: "October 3, 2025",
    ...
    events: [
      {
        ...
        Key: "Info Session",
        Point:
          "Tech Winter Break Introduction to Solutions Challenge – GDG On Campus SKNCOE",
        photos: [
          "assets/images/img1.jpg",
          "assets/images/img2.jpg",
          "assets/images/img1.jpg",
...
const PastEvents = () => {
  ...
  const currentYearData = EventsDetail.find((item) => item.year === activeYear);

  // The repeating sequence of Google colors
  const colorClasses = [
    styles.blueCard,
    styles.redCard,
    styles.yellowCard,
  ...
  return (
    <section className={styles.pastEvents}>
      <div className={styles.container}>
        <h1 className={styles.title}>Event Schedule</h1>

        <div className={styles.yearTabs}>
          {[2024, 2025, 2026].map((year) => (
            <button
              key={year}
              className={`${styles.yearBtn} ${activeYear === year ? styles.activeYear : ""}`}
        ...
        <div className={styles.timelineContainer}>
          ...
          <div className={styles.eventsList} key={activeYear}>
            {currentYearData?.events.map((event, index) => (
              <div
                key={index}
                className={`${styles.eventWrapper} ${index % 2 === 0 ? styles.left : styles.right}`}
                style={{ "--delay": index }}>
                <div className={styles.dot}></div>

                <div className={`${styles.card} ${colorClasses[index % 4]}`}>
                  <div className={styles.cardHeader}>
                    <span className={styles.timeTag}>{event.Date}</span>
                  </div>
                  <h3 className={styles.eventKey}>{event.Key}</h3>
                  <p className={styles.eventPoint}>{event.Point}</p>
                </div>
              </div>
            ))}
          </div>
